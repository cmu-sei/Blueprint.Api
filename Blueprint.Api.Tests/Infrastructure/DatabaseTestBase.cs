...
public abstract class DatabaseTestBase(DatabaseFixture fixture) : IAsyncLifetime
{
    ...
    /// cancel a test that hangs, which matters more here than in a pure unit test: blueprint's service
    /// layer holds explicit transactions, so a query blocked on a PostgreSQL lock would otherwise hold
    /// the whole run open.
    /// </summary>
    ...
    /// </summary>
    protected BlueprintContext Db { get; private set; }

    /// <summary>
    /// The substituted mediator that <c>BlueprintContext.PublishEventsAsync</c> resolves for
    /// <see cref="Db"/> and <see cref="NewContext"/>. Assert on it to verify published entity events.
    /// </summary>
    /// <remarks>
    /// Requests do not publish to this one. <see cref="BlueprintAppFactory"/> hands the request scope to
    /// the context it creates, so an entity event raised by a request reaches the application's real
    /// handlers - which is the point of driving the API over HTTP, and is what
    /// <see cref="BlueprintAppFactory.Hub"/> records.
    /// </remarks>
    ...
    /// </remarks>
    protected BlueprintContext NewContext() => Session.CreateContext();

    /// <summary>
    /// Seeds a user and their memberships. See <see cref="TestActorBuilder"/> for what an actor is and
    /// why blueprint tests act as one rather than as a hand-built principal.
    /// </summary>
