...
/// <remarks>
/// <para>
/// The application's own <see cref="Blueprint.Api.Data.BlueprintContext"/> registration cannot be
/// reused for this: <c>AddEventPublishingDbContextFactory</c> builds one pooled set of
/// <c>DbContextOptions</c> when the container is built, with one connection string baked in, so there is
/// no point at which a per-request database could be chosen. <see cref="BlueprintAppFactory"/> replaces
/// the scoped registration with one that asks this class.
/// </para>
/// <para>
