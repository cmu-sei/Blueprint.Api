...
namespace Blueprint.Api.Services
{
    ...
    {
        Task<IEnumerable<ViewModels.CiteRole>> GetTemplatesAsync(CancellationToken ct);
    ...
    public class CiteRoleService : ICiteRoleService
    {
        ...
        public async Task<IEnumerable<ViewModels.CiteRole>> GetTemplatesAsync(CancellationToken ct)
        {
            if (!(await _authorizationService.AuthorizeAsync(_user, null, new BaseUserRequirement())).Succeeded)
                throw new ForbiddenException();

            var citeRoleEntities = await _context.CiteRoles
                .Where(citeRole => citeRole.IsTemplate)
                .ToListAsync(ct);
