...
namespace Blueprint.Api.Services
{
    public interface IMselUnitService
    {
        ...
        Task<bool> DeleteByIdsAsync(Guid mselId, Guid unitId, bool hasSystemPermission, CancellationToken ct);
    }
    ...
    public class MselUnitService : IMselUnitService
    {
        ...
        public MselUnitService(BlueprintContext context, IPrincipal user, ILogger<IMselUnitService> logger, IMapper mapper)
        {
            _context = context;
        ...
        public async Task<ViewModels.MselUnit> CreateAsync(ViewModels.MselUnit mselUnit, bool hasSystemPermission, CancellationToken ct)
        {
        ...
        {
            // user must be a Content Developer or a MSEL owner
            if (!hasSystemPermission && !(await MselOwnerRequirement.IsMet(_user.GetId(), mselUnit.MselId, _context)))
                throw new ForbiddenException();
        ...
        public async Task<bool> DeleteAsync(Guid id, bool hasSystemPermission, CancellationToken ct)
        {
            ...
            // user must be a Content Developer or a MSEL owner
            if (!hasSystemPermission && !(await MselOwnerRequirement.IsMet(_user.GetId(), mselUnitToDelete.MselId, _context)))
                throw new ForbiddenException();
        ...
        public async Task<bool> DeleteByIdsAsync(Guid mselId, Guid unitId, bool hasSystemPermission, CancellationToken ct)
        {
