...
| Defect | Pinned by |
| --- | --- |
| Five entities carry a nullable `MselId` and a row without one is broadcast to a group named by the **empty string** — a name no connection can hold, so only the admin group hears about a template. `CardTeamHandler` reaches the same state by another route, its lookup selecting a `Guid?` that a missing card leaves null. | `EventHandlerTests.ARowWithNoMsel_IsBroadcastToAGroupNamedByTheEmptyString` |
| `DataValueHandler` and `PlayerApplicationTeamHandler` select a non-nullable `Guid`, so a row whose parent is gone is broadcast to the group named by all-zeros. Two ways of spelling "nobody", neither logged. | `EventHandlerTests.ARowWhoseParentIsGone_IsBroadcastToTheAllZeroGuidsGroup` |
| `CiteActionHandler`, `CiteDutyHandler`, `TeamUserHandler` and `UnitUserHandler` dereference an unchecked `SingleOrDefaultAsync`, so a row deleted between the save and the broadcast is a `NullReferenceException` raised inside `SaveChanges` — the request that saved the row answers 500 although its write committed. | `EventHandlerTests.TheFamiliesThatReReadTheRow_ThrowWhenItIsGone` |
| `TeamUserHandler`'s re-read `Include`s only `User` and then dereferences five navigations off `Team`, so whether a team-user broadcast succeeds depends on whether the request that saved the row also happened to load the team into the same change tracker. | `EventHandlerTests.ATeamUser_IsOnlyBroadcastableWhileItsTeamIsTracked` |
| `DataFieldHandler` maps whatever its re-read found, so a column deleted between the save and the broadcast is a message with a `null` payload rather than no message. | `EventHandlerTests.ADataFieldThatIsGone_IsBroadcastAsANullPayload` |
| The MSEL re-read filters `UserMselRoles` to the acting user, so the roles every client is told about depend on who saved the MSEL — the UI's list of who may do what is replaced, on every update, with a list of one. | `EventHandlerTests.AMselIsBroadcastWithTheActingUsersRolesAlone` |
| `UserHandler` addresses the creator's own user group and never `MainHub.USER_GROUP`, which exists for exactly this — so the administration screen listing users is not told when one is added, renamed or deleted. An auto-provisioned user has no creator, so its broadcast goes to the all-zeros group. | `EventHandlerTests.TheUserFamily_TellsTheCreatorsOwnGroupAndNotTheUserAdminGroup` |
| `CatalogHandler` addresses the units a catalog is shared with and never a group named by the catalog, so a client watching a catalog it is not shared through learns nothing about it; a catalog shared with no unit reaches the admin group alone. It also drops the `CancellationToken`. | `EventHandlerTests.TheCatalogFamily_TellsTheUnitsItIsSharedWithAndNeverTheCatalog` |
| `InjectHandler` and `InjectTypeHandler` address the admin group alone, so the content-developer screens are the only ones ever told about the reusable content a catalog is built from. | `EventHandlerTests.TheInjectFamilies_TellNobodyButTheAdminGroup` |
| `TeamUserHandler` and `UnitUserHandler` send the whole entity where the other 22 families send a mapped view model, and send it on delete too where the other 22 send an id — so a client cannot read the two messages the same way. Both inject an `IMapper` they never use. | `EventHandlerTests.TheMembershipFamilies_SendTheWholeEntityWhereEveryOtherFamilySendsAViewModel` |
| `SystemRoleHandler`, `GroupHandler` and `GroupMembershipHandler` send two arguments where the other 21 families send three, so a client written against the common shape reads the wrong slot and a role or group update cannot say what changed. | `EventHandlerTests.TheInstallationWideFamilies_NameNoModifiedPropertiesEvenOnAnUpdate` |
...
| Defect | Pinned by |
| --- | --- |
| `[SanitizeHtml]` is on six properties, and the set is not the set of strings a browser renders as HTML: `MselEntity.Description` is the exercise's own summary, `CardEntity.Description` and `DataValueEntity.Value` are pushed to Gallery, which renders an article body, and `MoveEntity.Description` sits beside the one move property that is attributed. | `SanitizerInterceptorTests.TheAttributeIsOnSixProperties_AndNotOnTheStringsThatReachGallery` |
| The interceptor does not check for null before sanitizing and Ganss answers the empty string, so saving a row with an attributed property unset writes `''` where the column would hold NULL — "never described" and "described, then cleared" become the same value. | `SanitizerInterceptorTests.ANullValue_IsStoredAsTheEmptyString` |
| Only `Added` and `Modified` entries are walked, so a hostile value that reached a column by another route — a restored backup, or the attribute being added later — is never cleaned up. The interceptor guards the write, not the table. | `SanitizerInterceptorTests.ARowThatIsMerelyTracked_IsNotSanitized` |

## Startup and middleware

| Defect | Pinned by |
| --- | --- |
| `ValidateModelStateFilter` is dead code. `BaseController` carries `[ApiController]` (`BaseController.cs:11`) and nothing configures `SuppressModelStateInvalidFilter`, so MVC's `ModelStateInvalidFilter` at order -2000 answers every invalid model before the global filter at order 0 — the `ApiError` shape titled "Invalid Data" has never reached a client. Delete the filter; the framework's `ValidationProblemDetails` is the better of the two answers. | `ErrorResponseTests.ABindingFailure_IsAProblemDocumentAndNotBlueprintsApiError`, `FilterTests.AnInvalidModel_IsA400CarryingTheFieldNames` |
| Every problem document answers `"status":"400"` — a JSON string where RFC 9457 section 3.1 requires a number — because `JsonIntegerConverter` is registered globally and `ProblemDetails.Status` is an `int?`. `ApiError.Status` is an `int`, so blueprint's own error shape does it too. | `ErrorResponseTests.TheProblemDocument_WritesItsStatusAsAJsonString`, `ErrorResponseTests.TheApiError_WritesItsStatusAsAJsonStringAsWell` |
| An unparseable id is refused against the **empty** key with the generic "The supplied value is invalid.", naming no field, where every other bad value names its JSON path. `Guid.Parse` is `JsonNullableGuidConverter`'s own call, so System.Text.Json does not mark its `FormatException` rethrowable and there is no path to attribute. | `ErrorResponseTests.AnUnparseableGuid_IsRefusedAgainstNoFieldAtAll`, `JsonConverterTests.ANullableGuidThatIsNotAGuid_IsAnUnwrappedFormatExceptionNamingNothing` |
| `JsonExceptionFilter`'s Production branch replaces `title` with "A server error occurred." and puts the real exception message in `detail`, so the message it set out to hide is one field to the right — and blueprint's unhandled exceptions are EF and Npgsql ones, so what a caller reads is a constraint, table or connection-string fragment. The filter has no logger, so withholding it loses the only copy. | `FilterTests.AServerErrorInProduction_HidesTheStackTraceAndLeaksTheMessage` |
| `ApiError` sets neither `type` nor `instance` and is served as `application/json`, so blueprint answers two error dialects that a client cannot discriminate by status — only by media type. | `FilterTests.AnErrorBody_NamesNeitherAProblemTypeNorTheRequest`, `ErrorResponseTests.AThrownExceptionIsBlueprintsApiError_NotAProblemDocument` |
| `ValidateModelStateFilter` reads `x.ErrorMessage` and falls back to `x.Exception.Message` with no null guard, so a model error carrying neither is a `NullReferenceException` inside the filter — the 400 it exists to produce becomes a 500. Latent: unreachable while MVC's filter answers first, and the first thing to appear if anybody suppresses it. | `FilterTests.AModelErrorWithNeitherAMessageNorAnException_Throws` |
| The `?bearer=` middleware finds the parameter with `SingleOrDefault`, so a url naming `bearer` twice is a 500 raised before routing and outside the MVC filters that would have made it an `ApiError`. `FirstOrDefault` is the whole fix. | `MiddlewareTests.TwoBearerParameters_AreA500` |
| The same middleware takes the value with `Split('=')[1]`, discarding everything from the second `=` — so a token with base64 padding is silently truncated and presented as a *different* credential, which authenticates as somebody else rather than failing. `Split('=', 2)[1]` is the fix. | `MiddlewareTests.ATokenWithATrailingEqualsSign_IsSilentlyTruncated` |
| It matches `StartsWith("bearer=")` with no comparison argument, so `?Bearer=` is not promoted and the refusal is indistinguishable from an expired token; and the promoted value is never URL-decoded, so a percent-encoded token arrives mangled. The decoding half is recorded rather than tested — any non-guid token is a 500 here for a different reason, so a test would pass for the wrong one. | `MiddlewareTests.ACapitalisedBearerParameter_IsNotPromoted` |
| `services.AddScoped<IInjectTypeService, InjectTypeService>();` appears verbatim at both `Startup.cs:228` and `Startup.cs:231`. Harmless — both descriptors name the same implementation — but it is the shape of line that makes a silent last-wins easy to write. | `CompositionTests.TwoServiceTypes_AreRegisteredTwice`, `CompositionTests.TheDuplicateRegistration_IsHarmless` |
| The provider `switch` at `Startup.cs:72-97` has no `default` arm, and it registers the health checks as well as the context — so a deployment whose `Database:Provider` is misspelled starts with no database registered and `/api/health/ready` answering Healthy, having nothing to check. Recorded rather than tested: the harness replaces the context registration, so a host built with a bogus provider behaves like a correct one. | `CompositionTests` class remarks |
| `MapHealthChecks` is called with no `HealthCheckOptions`, so the probes answer .NET 8+'s default JSON writer — which puts a 24-byte body inside the compression `MimeTypes` list and spends a gzip stream on it several times a minute. Costs nothing only because Kubernetes sends no `Accept-Encoding`. | `MiddlewareTests.AHealthProbe_IsGzippedBecauseItAnswersJson` |
| One health check answers both probes (`AddNpgSql(..., tags: ["ready", "live"])`), so liveness and readiness are the same question and a database outage makes Kubernetes restart the pod rather than take it out of the load balancer. | `MiddlewareTests.BothProbes_AnswerTheSameSingleCheck` |
| `JsonDoubleConverter` is configuration with no subject: no view model or entity property in the API is a `double`, `float` or `decimal`. Its three defects are therefore latent — NaN is written as `0.0`, indistinguishable from a measurement of zero; infinity is unguarded and throws *while writing*, after the response has begun; and the string form is parsed with no `CultureInfo`, so `"1,5"` means fifteen or nothing depending on the server's locale. | `JsonConverterTests.NothingInTheApiIsAFloatingPointNumber`, `.ADoubleThatIsNaN_IsWrittenAsZero`, `.ADoubleThatIsInfinite_ThrowsWhileWriting`, `.ADoubleAsAString_IsParsedInTheServersCulture` |
